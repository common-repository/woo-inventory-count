...
function wcic_woopos_show_page() {  global $_wcic_woopos_log, $_wcic_woopos_prods_count; wcic_woopos_enqueue(); ?>
    <div id="wcic_woopos_main" align="left">
      <h1 style="letter-spacing:0.1em"><a href="https://woopos.com/" target="_blank" rel="nofollow"><img align="middle" src="https://support.woopos.com/wp-content/uploads/2017/01/woopos-logo-150px.png" style="height:40px" /></a> WooCommerce Inventory Count</h1> 
      <form method="post">
        ...
        <div style="float:left">
         <?php if (!empty($_wcic_woopos_log['success']) && count($_wcic_woopos_log['success']) > 0 && (empty($_wcic_woopos_log['error']) || count($_wcic_woopos_log['error']) == 0)) { ?>
...
function wcic_woopos_generate_qr_code($wooprods) {
    ...
    for ($bn = 0; $finish != true ; $bn++) {
        ...
        if (strlen($block_data) == 0) {
            ...
            $request_url = "https://mywoopos.com/POSLSService.asmx/SaveFileFromBase64?BytesToBase64={$block_data}&FileName=SKULIST_{$filename}.TXT&BlockNumber={$bn}&BlockSize={$block_bytes}";
...
function wcic_woopos_download_server_data($file_name, $bytes) {
    ...
    for ($i = 0; $i <= $blocks; $i++) {
        $down_link = "https://mywoopos.com/POSLSService.asmx/GetFileBase64?FileName=" . $file_name . "&BlockNumber={$i}&BlockSize=900";
...
if(count($vari)>0){
		...
		if (isset($pM['_ywbc_barcode_value']) == true && $pM['_ywbc_barcode_value'][0] != "") {
		 	$sku = strtoupper($pM['_ywbc_barcode_value']);
              		$wooprods[$sku]['id'] = $product->ID;
                	$wooprods[$sku]['q'] = round($pM['_stock'][0]);
                	$wooprods[$sku]['sku'] = $sku;
                	$wooprods[$sku]['p'] = @number_format($pM['_regular_price'][0], 2);
 		...
 		if (isset($pM2['_cpf_ean']) == true && $pM2['_cpf_ean'][0] != "") {
	      		$vsku = strtoupper($pM2['_cpf_ean']);
              		$wooprods[$vsku]['id'] = $var->ID;
                    	$wooprods[$vsku]['q'] = round($pM2['_stock'][0]);
                    	$wooprods[$vsku]['sku'] = $vsku;
                    	$wooprods[$vsku]['p'] = $vsku_price;
 		...
 		if (isset($pM2['_cpf_upc']) == true && $pM2['_cpf_upc'][0] != "") {
	      		$vsku = strtoupper($pM2['_cpf_upc']);
              		$wooprods[$vsku]['id'] = $var->ID;
                    	$wooprods[$vsku]['q'] = round($pM2['_stock'][0]);
                    	$wooprods[$vsku]['sku'] = $vsku;
                    	$wooprods[$vsku]['p'] = $vsku_price;
 		...
 		if (isset($pM2['_woosea_upc']) == true && $pM2['_woosea_upc'][0] != "") {
	      		$vsku = strtoupper($pM2['_woosea_upc']);
              		$wooprods[$vsku]['id'] = $var->ID;
                    	$wooprods[$vsku]['q'] = round($pM2['_stock'][0]);
                    	$wooprods[$vsku]['sku'] = $vsku;
                    	$wooprods[$vsku]['p'] = $vsku_price;
 		...
 		if (isset($pM2['_woosea_ean']) == true && $pM2['_woosea_ean'][0] != "") {
	      		$vsku = strtoupper($pM2['_woosea_ean']);
              		$wooprods[$vsku]['id'] = $var->ID;
                    	$wooprods[$vsku]['q'] = round($pM2['_stock'][0]);
                    	$wooprods[$vsku]['sku'] = $vsku;
                    	$wooprods[$vsku]['p'] = $vsku_price;
 		...
 		if (isset($pM2['_ywbc_barcode_value']) == true && $pM2['_ywbc_barcode_value'][0] != "") {
	      		$vsku = strtoupper($pM2['_ywbc_barcode_value']);
              		$wooprods[$vsku]['id'] = $var->ID;
                    	$wooprods[$vsku]['q'] = round($pM2['_stock'][0]);
                    	$wooprods[$vsku]['sku'] = $vsku;
                    	$wooprods[$vsku]['p'] = $vsku_price;
 		...
 		if (isset($pM2['UPC']) == true && $pM2['UPC'][0] != "") {
	      		$vsku = strtoupper($pM2['UPC']);
              		$wooprods[$vsku]['id'] = $var->ID;
                    	$wooprods[$vsku]['q'] = round($pM2['_stock'][0]);
                    	$wooprods[$vsku]['sku'] = $vsku;
                    	$wooprods[$vsku]['p'] = $vsku_price;
 		...
 		if (isset($pM2['_wpm_gtin_code']) == true && $pM2['_wpm_gtin_code'][0] != "") {
	      		$vsku = strtoupper($pM2['_wpm_gtin_code']);
              		$wooprods[$vsku]['id'] = $var->ID;
                    	$wooprods[$vsku]['q'] = round($pM2['_stock'][0]);
                    	$wooprods[$vsku]['sku'] = $vsku;
                    	$wooprods[$vsku]['p'] = $vsku_price;
 		...
 		if (isset($pM2['_pp_sku_0']) == true && $pM2['_pp_sku_0'][0] != "") {
	      		$vsku = strtoupper($pM2['_pp_sku_0']);
              		$wooprods[$vsku]['id'] = $var->ID;
                    	$wooprods[$vsku]['q'] = round($pM2['_stock'][0]);
                    	$wooprods[$vsku]['sku'] = $vsku;
                    	$wooprods[$vsku]['p'] = $vsku_price;
 		...
 		if (isset($pM2['wpfoof-gtin-name']) == true && $pM2['wpfoof-gtin-name'][0] != "") {
	      		$vsku = strtoupper($pM2['wpfoof-gtin-name']);
              		$wooprods[$vsku]['id'] = $var->ID;
                    	$wooprods[$vsku]['q'] = round($pM2['_stock'][0]);
                    	$wooprods[$vsku]['sku'] = $vsku;
                    	$wooprods[$vsku]['p'] = $vsku_price;
	...
	if ( !session_id()) session_start();
        ...
        if (isset($_REQUEST['method'])) {
            ...
            switch ($_REQUEST['method']) {
                case 'ready_check':
                    $fl = strip_tags(file_get_contents("https://mywoopos.com/POSLSService.asmx/GetFileLen?FileName={$_GET['filename']}"));
                    if ($fl > 0) {
                        echo $fl;
                    } else {
                        echo "Waiting...";
